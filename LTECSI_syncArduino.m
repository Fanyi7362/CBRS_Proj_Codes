...
for i = 1:length(radiolist)
  if strcmp(radiolist(i).Status, 'Success')
    if (strcmp(radiolist(i).Platform, 'X300') || ...
        ...
        radio.DecimationFactor = 8;        % Sampling rate is 20e6
        radioFound = true;
        break;
    end    
...
trace_1 = reshape(burstCaptures,[],1);
clear burstCaptures
...
while (size(rxgrid,2) > 0)
    ...
    hest_array(:,Nsymbols:(Nsymbols+L*Nsubframes_perEst-1)) = hest(:,:,1,1);
